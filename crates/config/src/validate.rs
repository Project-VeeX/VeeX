...
use crate::{
    defaults::{DEFAULT_DIRECT_OUTBOUND_TAG, DEFAULT_LOG_LEVEL},
...
pub fn parse_config(input: &str) -> Result<ProxyConfig, ConfigError> {
    ...
    let log = parse_log(root.get("log"))?;
    let inbounds = parse_inbounds(root.get("inbounds"))?;
    let outbounds = parse_outbounds(root.get("outbounds"))?;
    let route = parse_route(root.get("route"))?;
