...
use veex_core::{DispatchOutbound, OutboundRegistry, ProxyError};
    ...
    for outbound in config.outbounds.iter().map(lower_outbound) {
        match outbound {
            LoweredOutbound::Direct(direct) => {
                ...
                let instance: Arc<dyn DispatchOutbound> = Arc::new(DirectOutbound::new(
                    direct.meta,
                    logger,
                    resolver_policy,
                    dialer,
            ...
            LoweredOutbound::Trojan(trojan) => {
                ...
                let instance: Arc<dyn DispatchOutbound> = Arc::new(TrojanOutbound::new(
                    trojan.meta,
                    logger,
                    resolver_policy,
                    dialer,
