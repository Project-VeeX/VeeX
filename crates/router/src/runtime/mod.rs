...
use crate::{
    ...
    rule::{RouteFinalAction, RouteUpgradeAction},
...
pub(crate) async fn route_stream(
    ...
    loop {
        ...
        let domain_before = route_input.domain;

        match router.step_from(route_input, next_index) {
            RouteStep::Miss {
                trace,
                reason,
                next_index: resume_at,
            } => {
                log_route_rule_eval(&ctx, trace, domain_before);
                log_route_rule_miss(&ctx, trace, domain_before, reason);
                next_index = resume_at;
            }
            RouteStep::Upgrade {
                trace,
                ...
                log_route_rule_eval(&ctx, trace, domain_before);
                log_route_rule_match(&ctx, trace, domain_before);

                match action {
                    RouteUpgradeAction::Sniff(action) => {
            ...
            RouteStep::Final { trace, decision } => {
                log_route_rule_eval(&ctx, trace, domain_before);
                log_route_rule_match(&ctx, trace, domain_before);
                log_route_final_selected(&ctx, trace, domain_before, &decision.final_action);
...
pub(crate) async fn route_packet(
    ...
    let decision = router.select(RouteInput::from_session(&ctx));
    Ok(RouteResult {
        ctx,
        decision,
        input,
