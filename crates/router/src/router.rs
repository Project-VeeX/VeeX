use crate::{
    ...
    rule::{
        ...
        RouteReason, RouteRule, RouteTarget,
    },
    runtime,
};
use veex_core::{
...
impl Router {
    ...
    pub fn select(&self, input: RouteInput<'_>) -> RouteDecision {
        ...
        loop {
            match self.step_from(input, next_index) {
                RouteStep::Miss {
    ...
    pub fn route_packet(
        ...
        input: PacketCarrier,
        ctx: SessionContext,
    ) -> impl std::future::Future<Output = Result<RouteResult<PacketCarrier>, RouteError>> + '_
    {
        runtime::route_packet(self, input, ctx)
    }

    pub fn step_from<'a>(&'a self, input: RouteInput<'_>, start_index: usize) -> RouteStep<'a> {
        let Some(rule) = self.rules.get(start_index) else {
...
mod tests {
    ...
    use crate::{
        RouteAction, RouteFinalAction, RouteInput, RouteReason, RouteRule, RouteStep, Router,
    };
    ...
    fn private_destinations_do_not_bypass_without_explicit_rule() {
        ...
        let decision = router.select(RouteInput::new(&destination, Some("test"), None));
        assert_eq!(decision.outbound_tag(), Some("proxy"));
        assert_eq!(decision.reason, RouteReason::Final);
    }
    ...
    fn matches_domain_suffix_for_apex_and_subdomain() {
        ...
        let apex_decision = router.select(RouteInput::new(&apex, Some("test"), Some("google.com")));
        let subdomain_decision = router.select(RouteInput::new(
            &subdomain,
            Some("test"),
            Some("www.google.com"),
        ));
    ...
    fn matches_private_loopback_and_link_local_rules_as_regular_rules() {
        ...
        assert_eq!(
            router
                .select(RouteInput::new(&loopback, Some("test"), None))
                .outbound_tag(),
            Some("loopback-direct")
        );
        assert_eq!(
            router
                .select(RouteInput::new(&private, Some("test"), None))
                .outbound_tag(),
            Some("private-direct")
        );
        assert_eq!(
            router
                .select(RouteInput::new(&link_local, Some("test"), None))
                .outbound_tag(),
    ...
    fn matches_port_and_inbound_rules() {
        ...
        let port_match = router.select(RouteInput::new(
            &port_destination,
            Some("redirect-in"),
            None,
        ));
        let inbound_match = router.select(RouteInput::new(
            &inbound_destination,
            Some("socks-in"),
            None,
        ));
    ...
    fn requires_all_matchers_in_a_rule_to_match() {
        ...
        let matching_decision = router.select(RouteInput::new(
            &matching,
            Some("test"),
            Some("www.google.com"),
        ));
        let mismatched_decision = router.select(RouteInput::new(
            &mismatched_port,
            Some("test"),
            Some("www.google.com"),
        ));
    ...
    fn can_return_hijack_dns_as_a_final_action() {
        ...
        let decision = router.select(RouteInput::new(&destination, Some("dns-in"), None));
    ...
    fn can_be_rerun_after_domain_becomes_available() {
        ...
        let first = router.select(RouteInput::new(&destination, Some("test"), None));
        let second = router.select(RouteInput::new(
            &destination,
            Some("test"),
            Some("www.google.com"),
        ));
    ...
    fn step_api_exposes_upgrade_without_executing_it() {
        ...
        let step = router.step_from(RouteInput::new(&destination, Some("tproxy-in"), None), 0);
