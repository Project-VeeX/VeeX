...
`direct` uses the shared resolve/connect semantics and then enters relay immediately. `trojan` keeps the same resolve/connect base semantics, adds TLS when enabled, then performs Trojan protocol request setup before relay. `connect_timeout` belongs to the connect stage; `disable_tcp_keep_alive` / `tcp_keep_alive` / `tcp_keep_alive_interval` belong to TCP dialing behavior; `tls.handshake_timeout` and `tls.alpn` belong only to the TLS stage.
