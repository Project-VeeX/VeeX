...
- VeeX currently has explicit stream and packet execution paths, but only a narrow packet-facing component surface centered on `direct-in -> direct-out`. Do not silently expand the work into a DNS platform, generalized UDP stack, TUN path, fake-ip system, generalized sniffing platform, or full rule engine.
